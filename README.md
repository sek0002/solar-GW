...
- `TESLA_PUBLIC_KEY_URL` (optional override; otherwise derived from the partner domain)
...
When running locally, the app can auto-generate a Tesla EC keypair and serve the expected public-key path from the same host as `TESLA_REDIRECT_URI`.
...
Vehicle-level data access may still require Tesla virtual-key pairing and, on some vehicles, enabling `Allow Third-Party App Data Streaming`.
